...
Before proving the claim let us list obvious properties of the functions $\alpha$ and~$\beta$. 
...
the maximum of restrictions $(\alpha-\lambda)|_D$ admits a maximum point on the boundary $\partial D$.
...
such that $\alpha(q)>\beta(p)+\eps$ for some fixed $\eps>0$.
...
We say that the value $t$ is right (left) if $a$ can be connected to $q$ by a curve in $\Theta_t$ which goes on right (correspondingly left) from $p$.
...
Since $\Sigma$ is saddle, a value $t$ can not be right and left at the same time --- if it is the case, then $\Pi_t$ cuts a hat from $\Sigma$.

The sets of right (left) values are open in $[0,1]$.
...
the slope of $\lambda$ and therefore $\lambda_t$ can be made arbitrary large;
...
With this definition, the saddle graph theorem, does not hold for saddle discs in the product space $C\times \RR$, where $C$ is a cone with large total angle --- the vertical line in the product space $C\times \RR$ might intersect the saddle disc along a nontrivial segment.
