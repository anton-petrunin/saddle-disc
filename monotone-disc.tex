...
\begin{abstract}
We prove an analog of Schoen and Yau's univalentness theorem for saddle maps between discs.
\end{abstract}
...
The proof of the baby theorem is as hard as the proof of the main theorem formulated below --- 
read the final remarks if it looks trivial to you.

A map $f$ from a closed disc $\DD$ to a surface $\Delta$ with a geodesic metric is called \emph{saddle} 
if for any geodesic $[x,y]$ each connected component of the complement $\DD\backslash f^{-1}[x,y]$ meets the boundary $\partial\DD$.
...
Assume $(\DD,|{*}-{*}|)$ is a disc with Riemannian metric such that any two points $x,y\in\DD$ are joined by unique geodesic $[x,y]$.
...
Let us show that for any $s$ the set $\Psi_s$ is either empty or simply connected.
...
By Claim \ref{claim}\textit{(ii)}, $\Psi_s$ is simply connected.
...
Let us present a \emph{fake} proof of the baby theorem (\ref{baby}) which is based on the idea from the problem ``Saddle surface'' 
in \cite{petrunin-orthodox}.
...
A correct proof of the baby theorem can be build on the theorem of Shefel, but it does not seem possible to generalize.
...
We do not know if this statement can be generalized to surfaces with $\CAT[0]$ metrics; 
