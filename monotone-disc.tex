...
In particular, it includes nonpositively curved surfaces considered in the original univalentness theorem in \cite{schoen-yau}
...
\[E(f)=\int_M|df|^2 \mathoperator{dvol},\]
...
\parbf{Remark.}
There is a well established theory of harmonic maps from Euclidean domains into spaces with upper curvature bounds
in the sense of Alexandrov, cf. \cite{GS} and \cite{KS}. In this more general setting harmonic maps are again defined as local minimizers of
an appropriate energy functional. The above proof remains valid and harmonic maps are still saddle.
...
Choose a chord $\gamma$ which passes $w$ and goes 
...
There is a continuous one parameter family of geodesics $\gamma_t$ passing $w$ with the ends at $\partial \DD$
...
Namely, there exists a harmonic map $f$ from a closed disc to a disc with a metric which is flat except for one cone 
point $q$ of negative curvature such that $f$ restricts to a homeomorphism between boundaries but $f^{-1}\{q\}$ is a nontrivial tree.

Therefore, the main theorem is optimal even for harmonic maps.
...
%a homeomorphism on $\partial D$. If we precompose $u|_D$ with a Riemann mapping $\rho:\DD\to D$, then we obtain a noninjective harmonic map between discs whose restriction to the boundary is a homeomorphism.
...
\textit{Metric minimzing surfaces revisited.} In preparation.
